  ...
  <el-aside class="index-aside" height="100vh" width="230px">
    <div class="index-aside-inner menulist">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item">
        <div class="menulistImg" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" background-color="#edf3fb" text-color="#4a5f79" active-text-color="#2b6cb0" default-active="0">
          ...
          <el-submenu
            v-if="item.frontMenu && item.frontMenu.length"
            :style="menulistBorderBottom"
            v-for="(menu, fIndex) in item.frontMenu"
            :key="'front-' + menu.menu + fIndex"
            :index="'f' + fIndex + item.backMenu.length + 2 + ''"
          >
            <template slot="title">
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "#dfe8f6"
          })
...
<style lang="scss" scoped>
  .index-aside {
    position: relative;
    overflow: hidden;
    border-right: 1px solid rgba(216, 228, 243, 0.9);
  ...
  .index-aside ::v-deep .el-menu-item,
  .index-aside ::v-deep .el-submenu__title {
    border-radius: 8px;
    margin: 4px 6px;
    height: 46px;
    line-height: 46px;
  ...
  .index-aside ::v-deep .el-submenu .el-menu-item {
    margin-left: 18px;
    min-width: 0;
  }
  ...
  @media (max-width: 980px) {
    .index-aside {
      box-shadow: 8px 0 24px rgba(30, 64, 124, 0.06);
    }
  }
</style>
